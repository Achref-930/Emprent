...
/**
 * Prevents the last two words of a name from wrapping apart — e.g.
 * "Oversized T Design 1" will never break as "...Design" / "1" on a
 * narrow screen. It joins them with a non-breaking space, so the browser
 * is still free to wrap anywhere *before* that pair (e.g. "Oversized T"
 * / "Design 1") when the full name doesn't fit on one line, but "Design"
 * and "1" always stay together as a unit.
 */
export function glueLastWords(name) {
